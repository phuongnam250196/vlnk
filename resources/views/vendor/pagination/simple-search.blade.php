@if ($paginator->hasPages())
    <ul class="page-numbers" role="navigation">
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <li class="page-item active" aria-current="page"><span aria-current="page" class="page-numbers current">{{ $page }}</span></li>
                    @else
        ...
        @if ($paginator->hasMorePages())
            <li class="page-item">
                <a class="page-numbers" href="{{ $paginator->nextPageUrl() }}" rel="next" aria-label="@lang('pagination.next')">→</a>
            </li>
        @else
